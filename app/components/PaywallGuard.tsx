...
export const UnlockModal = ({ isOpen, onClose, onUnlock }: any) => {
  ...
  // 1. Existing Manual Unlock Logic
  ...
  const handleStartTrial = async () => {
    ...
    try {
      ...
      let deviceId = localStorage.getItem('decision_app_device_id');
      if (!deviceId) {
          deviceId = crypto.randomUUID();
          localStorage.setItem('decision_app_device_id', deviceId);
      }
      ...
      if (data.success) {
        // Success! Save the trial code as if they typed it
        localStorage.setItem('decision_app_license_code', data.code);
        
        // Optional: Trigger Confetti if you want
        const confetti = (await import('canvas-confetti')).default;
        confetti({ particleCount: 150, spread: 70, origin: { y: 0.6 } });

        alert(`Trial Started! You have 48 hours.\nCode: ${data.code}`);
        window.location.reload(); // Reload to unlock everything
      } else {
        setError(data.message || 'Trial failed');
      }
    } catch (err) {
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
      <div className="bg-[#121212] border border-white/10 w-full max-w-sm p-6 rounded-3xl shadow-2xl">
        ...
        <div className="mt-6 pt-6 border-t border-white/5">
            <p className="text-xs text-center text-slate-500 mb-3 uppercase tracking-wider font-bold">Not ready to buy?</p>
            <button 
              onClick={handleStartTrial}
              disabled={loading}
              className="w-full py-3 rounded-xl border border-blue-500/30 text-blue-400 hover:bg-blue-500/10 text-xs font-bold uppercase tracking-widest transition-all"
            >
