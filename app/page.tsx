...
// (Outside the function, near the imports)
                  ...
                  {menuView === 'main' && (
                      <>
                        <PaywallGuard isLocked={status !== 'full' && status !== 'trial' && status !== 'dev'} triggerUnlock={() => setUnlockModalOpen(true)}
                          >
                          <button onClick={() => setMenuView('edit')} className={styles.menuBtn}>
                            <span>Edit System</span> <span className="opacity-50">→</span>
                          </button>
                      </PaywallGuard>
                              ...
                              <AnimatePresence>
                                {expandedCat === cat.name && (
                                  ...
                                  >
                                    ...
                                    <PaywallGuard 
                                      isLocked={
                                        !['Meals', 'Self Care', 'Self-Care'].includes(cat.name) && 
                                        status !== 'full' && 
                                        status !== 'trial' && 
                                        status !== 'dev' // <--- ADD THIS!
                                      } 
              ...
              {!selectedCategory && !isDone && (
                <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="mt-32 w-full max-w-sm flex flex-col items-center">
                  ...
                  <div className="w-full space-y-4">
                    {/* SAFEGUARD: Only map if categories exist */}
                    {categories?.length > 0 && categories.map((cat, i) => (
                      <PaywallGuard 
                        key={cat.name} /* <--- THIS WAS THE MISSING KEY! */
                        isLocked={
                          !['Meals', 'Self Care', 'Self-Care'].includes(cat.name) && 
                          status !== 'full' && 
                          status !== 'trial' && 
